...
export function SignUp() {
  ...
  const emailBody = `Please fill out the following details to request a new account:
...
Email Address: `;
  ...
  // Error handling is crucial in case the variable isn't set
  if (!adminEmail) {
    return (
  ...
  const handleCopy = () => {
    navigator.clipboard
      ...
      .then(() => {
        toast.success("Full sign-up details have been copied to your clipboard.");
      })
      .catch((err) => {
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center h-screen bg-background">
        <Card className="w-full max-w-md">
          <CardHeader>
            <CardTitle className="text-2xl">New Account Registration</CardTitle>
            <CardDescription>
              To request a new account, please send an email with the required details to the
              administrator at: <br />
          ...
          <CardContent>
            <div className="flex flex-col space-y-4">
              <p className="text-sm text-muted-foreground">
                Click the button below to open your default email client with a template, or copy
                the full details to paste manually.
              </p>
              <a href={mailtoLink} className="w-full">
                <Button className="w-full">Request Account via Email</Button>
              </a>
